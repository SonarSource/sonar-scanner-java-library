...
public class BootstrapIndexDownloaderTest {
  ...
  public void should_download_jar_files() throws Exception {
    ...
    when(connection.callServerApi("/batch/index")).thenReturn(
    ...
    verify(connection, times(1)).callServerApi("/batch/index");
  }

  @Test
  public void test_invalid_index() throws Exception {
    when(connection.callServerApi("/batch/index")).thenReturn("cpd.jar\n");
  ...
  public void should_fail() throws IOException {
    when(connection.callServerApi("/batch/index")).thenThrow(new IOException("io error"));
